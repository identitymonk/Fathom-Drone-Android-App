    ...
    throws IOException
  {
    ...
    while (true)
    {
      ...
      if ((i != 0) && ("gzip".equalsIgnoreCase(paramChain.header("Content-Encoding"))) && (HttpHeaders.hasBody(paramChain)))
      {
        ...
        paramChain = paramChain.headers().newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build();
        ((Response.Builder)localObject1).headers(paramChain);
        ((Response.Builder)localObject1).body(new RealResponseBody(paramChain, Okio.buffer((Source)localObject2)));
      }
      return ((Response.Builder)localObject1).build();
      label368: ((Request.Builder)localObject2).header("Transfer-Encoding", "chunked");
