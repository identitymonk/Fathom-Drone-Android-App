  ...
  implements Animatable
{
  ...
  {
    Ring localRing = this.mRing;
    1 local1 = new Animation(localRing)
    {
      public void applyTransformation(float paramFloat, Transformation paramTransformation)
      {
        if (MaterialProgressDrawable.this.mFinishing)
        {
          MaterialProgressDrawable.this.applyFinishTranslation(paramFloat, this.val$ring);
          return;
        }
        float f1 = MaterialProgressDrawable.this.getMinProgressArc(this.val$ring);
        ...
        MaterialProgressDrawable.this.updateRingColor(paramFloat, this.val$ring);
        if (paramFloat <= 0.5F)
        {
        ...
        if (paramFloat > 0.5F)
        {
          ...
          this.val$ring.setEndTrim(f2 + f4 * (0.8F - f1));
        }
    ...
    local1.setAnimationListener(new Animation.AnimationListener(localRing)
    {
