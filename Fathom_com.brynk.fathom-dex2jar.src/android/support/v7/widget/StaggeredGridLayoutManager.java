...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager
{
  ...
  private int fill(RecyclerView.Recycler paramRecycler, LayoutState paramLayoutState, RecyclerView.State paramState)
  {
    ...
    label158: label169: label190: label223: int i2;
    int m;
    int n;
    ...
    if (this.mLayoutState.mInfinite)
      if (paramLayoutState.mLayoutDirection == 1)
      {
        ...
        label371: i2 = j - this.mSecondaryOrientation.getDecoratedMeasurement(localView);
        i1 = j;
        j = i2;
        if (this.mOrientation != 1)
          break label866;
        layoutDecoratedWithMargins(localView, j, m, i1, n);
        label415: if (!localLayoutParams.mFullSpan)
          break label883;
        updateAllRemainingSpans(this.mLayoutState.mLayoutDirection, i);
        label436: recycle(paramRecycler, this.mLayoutState);
    ...
    while (true)
    {
      ...
      break;
      label527: k = this.mPrimaryOrientation.getStartAfterPadding();
      break label62;
      label539: i1 = 0;
      break label137;
      label545: localSpan = getNextSpan(paramLayoutState);
      break label158;
      label555: localSpan = this.mSpans[j];
      break label169;
      label567: addView(localView, 0);
      break label190;
      label577: j = localSpan.getEndLine(k);
      break label223;
      label589: if (localLayoutParams.mFullSpan);
      ...
      label701: if (paramLayoutState.mLayoutDirection == 1)
      {
        if (!areAllEndsEqual());
        for (j = 1; ; j = 0)
        {
      ...
      label779: j = this.mSecondaryOrientation.getEndAfterPadding() - (this.mSpanCount - 1 - localSpan.mIndex) * this.mSizePerSpan;
      break label371;
      label809: if (localLayoutParams.mFullSpan);
      ...
      label866: layoutDecoratedWithMargins(localView, m, j, n, i1);
      break label415;
      label883: updateRemainingSpans(localSpan, this.mLayoutState.mLayoutDirection, i);
      break label436;
      label901: this.mRemainingSpans.set(localSpan.mIndex, false);
    }
    label917: if (j == 0)
  ...
  private void onLayoutChildren(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, boolean paramBoolean)
  {
    ...
    label575: label592: for (int j = k; ; j = 0)
    {
      i = m;
      if (j != 0)
      {
  ...
  private void repositionToWrapContentIfNecessary()
  {
    ...
    int i = 0;
    label167: LayoutParams localLayoutParams;
    if (i < k)
    {
      localView = getChildAt(i);
      localLayoutParams = (LayoutParams)localView.getLayoutParams();
    ...
    while (true)
    {
      ...
      break label167;
      break;
      label209: if ((isLayoutRTL()) && (this.mOrientation == 1))
      {
  ...
  private void updateLayoutState(int paramInt, RecyclerView.State paramState)
  {
    ...
    label132: label184: label190: label234: for (boolean bool1 = bool2; ; bool1 = false)
    {
  ...
  View hasGapsToFix()
  {
    ...
    label283: label351: return (View)null;
  }
  ...
  public Parcelable onSaveInstanceState()
  {
    ...
    while (true)
    {
      ...
      break;
      label240: i = getFirstChildPosition();
      break label130;
      label248: k = this.mSpans[j].getStartLine(-2147483648);
      i = k;
      if (k == -2147483648)
        continue;
      i = k - this.mPrimaryOrientation.getStartAfterPadding();
    }
    label281: localSavedState.mAnchorPosition = -1;
      ...
      else
      {
        ...
        if (paramInt2 >= 0)
        {
          ...
          if (localView.isFocusable())
          {
            ...
            {
              i = 1;
              label166: if (StaggeredGridLayoutManager.this.mReverseLayout)
                break label205;
            }
