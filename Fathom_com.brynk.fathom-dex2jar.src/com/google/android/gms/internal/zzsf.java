...
public class zzsf<R extends Result> extends TransformedResult<R>
    ...
    while (true)
    {
      synchronized (this.yH)
      {
        if (paramR.getStatus().isSuccess())
        {
          if (this.Bv == null)
            continue;
          zzry.zzatf().submit(new Runnable(paramR)
          {
            @WorkerThread
            public void run()
            {
